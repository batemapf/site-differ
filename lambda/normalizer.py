...
def normalize_html(html: str, selector: Optional[str] = None, 
        ...
        try:
            ...
            if selected:
                ...
                for elem in selected:
                    soup.append(elem)
                logger.debug(f"Applied selector '{selector}', found {len(selected)} elements")
            else:
                logger.warning(f"Selector '{selector}' matched no elements, using full page")
        except Exception as e:
            logger.warning(f"Selector '{selector}' failed: {e}, using full page")
    ...
    logger.debug(f"Normalized text length: {len(normalized)} chars, {len(lines)} lines")
