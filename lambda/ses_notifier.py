...
def send_digest_email(changes: List[Dict[str, Any]], total_urls: int) -> None:
    ...
    to_addresses = [addr.strip()
                    for addr in to_addresses_str.split(',') if addr.strip()]

    if not from_address or not to_addresses:
        raise ValueError(
            "SES_FROM and SES_TO environment variables must be set")

    # Generate email subject
    now = datetime.now(timezone.utc)
    subject = f"Website changes detected ({
        len(changes)} of {total_urls}) - {
    ...
    try:
        ...
        logger.info(
            f"Email sent successfully: MessageId={
                response['MessageId']}")
...
def generate_text_body(changes: List[Dict[str, Any]], total_urls: int,
    ...
    for i, change in enumerate(changes, 1):
        ...
        if change['is_new']:
            ...
            lines.append(
                f"   Previous hash: {change['previous_hash'][:16]}...")
...
def generate_html_body(changes: List[Dict[str, Any]], total_urls: int,
    ...
    html_parts = [
        ...
        ".change { border: 1px solid #bdc3c7; border-radius: 5px; "
        "padding: 15px; margin: 20px 0; }",
        ".change-header { font-weight: bold; color: #2980b9; "
        "margin-bottom: 10px; }",
        ".url { color: #3498db; word-break: break-all; }",
        ".status { color: #27ae60; font-weight: bold; }",
        ".diff { background: #f8f9fa; padding: 10px; "
        ...
        "<div class='summary'>",
        f"<p><strong>Run time:</strong> "
        f"{run_time.strftime('%Y-%m-%d %H:%M:%S UTC')}</p>",
    ...
    for i, change in enumerate(changes, 1):
        html_parts.append("<div class='change'>")
        html_parts.append(
            f"<div class='change-header'>{i}. Change Detected</div>")
        escaped_url = escape_html(change['url'])
        html_parts.append(
            f"<p><strong>URL:</strong> <a href='{escaped_url}' "
            f"class='url'>{escaped_url}</a></p>")

        if change['is_new']:
            html_parts.append("<p class='status'>Status: NEW URL</p>")
        else:
            html_parts.append(
                f"<p><strong>Previous hash:</strong> "
                f"<code>{change['previous_hash'][:16]}...</code></p>")
            html_parts.append(
                f"<p><strong>New hash:</strong> "
                f"<code>{change['new_hash'][:16]}...</code></p>")
        ...
        for line in change['diff_snippet'].splitlines():
            if line.startswith('+'):
                html_parts.append(
                    f"<span class='added'>{
                        escape_html(line)}</span>")
            elif line.startswith('-'):
                html_parts.append(
                    f"<span class='removed'>{
                        escape_html(line)}</span>")
            else:
    ...
    html_parts.append(
        "<p>This is an automated message from Website Diff Checker.</p>")
    html_parts.append("</div>")
